...
if sys.version_info < (3,):
    from . import images2gif_py2 as images2gif
else:
    from . import images2gif_py3 as images2gif
...
def legofy_image(base_image, brick_image, output_path, size, palette_mode, dither):
    ...
    if output_path: make_lego_image(base_image, brick_image).save(output_path)
    else: return make_lego_image(base_image, brick_image)
...
def main(image_path, output_path=None, size=None,
    ...
    if image_path.lower().endswith(".gif") and base_image.is_animated:
        ...
        if output_path is None:
            return legofy_image(base_image, brick_image, output_path, size, palette_mode, dither)
